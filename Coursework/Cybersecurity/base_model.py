...
base_model.py is the script that loads the prepared dataset (generated in the Colab Notebook) for fine tuning and evaluation with an appended classification head. The base models are loaded from hugging face, fine-tuned on the TruthSeeker dataset with the conditional labels as targets (True + Agree = True and False + Disagree = True, True + Disagree = False and False + Agree = False), and evaluated on accuracy, precision, recall, and F1. 
"""
...
def tokenize_data(dataset, tokenizer, model_name=None):
    ...
    if model_name and 'bertweet' in model_name.lower():
        ...
        def tokenize_bertweet(examples):
            ...
            # Then tokenize with normalization=True for BERTweet
            return tokenizer(
                normalized_texts,
                truncation=True,
                max_length=128,  # BERTweet typically uses shorter sequences due to tweet length limits
                padding='max_length'
            )
        ...
        tokenized_data = dataset.map(
            lambda x: tokenizer(x['text'], truncation=True, max_length=300, padding='max_length'),
            batched=True
        )
...
def train_model(model_name, dataset):
    ...
    # Tokenizer & model
    logger.info(f"Loading model and tokenizer: {MODEL_MAP[model_name]}")
    ...
    plot_roc(preds, labels, model_dir, model_name=model_name)
    ...
    # Push model and relevant files to Hugging Face Hub
    # try:
    #     trainer.push_to_hub()
    # except Exception as e:
    #     logger.info(f"failed to push model to hub with error: {e}")
...
def main():
    ...
    if len(all_model_results) > 0:
        ...
        for model_name, results in all_model_results.items():
            test_metrics = results['test_results']  # Changed from 'metrics']['test']
